...
const fs = require('fs');
console.log('File .env exists:', fs.existsSync('./.env'));
console.log('Content .env:', fs.readFileSync('./.env', 'utf8'));

require('dotenv').config({ path: './.env', override: true });
console.log('Loade variables:',Object.keys(process.env));
console.log('CHAT_ID:',process.env.CHAT_ID);

const botToken = process.env.BOT_TOKEN;
const chatId = process.env.CHAT_ID?.trim();

console.log('CHAT_ID value:', JSON.stringify(process.env.CHAT_ID));
